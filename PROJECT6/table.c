...
void quicksort (int (*compare)(),void **arr, int start, int end);
int partition (int (*compare)(), void **arr, int start, int end);
...
static int search(SET *sp, void *elt, bool *found) // big o of o(n)
{
...
void *getElements(SET *sp) // big o of o(n)
{
    ...
    quicksort(sp->compare,elts,0,sp->count-1);
    return elts;
}

int partition ( int(*compare)(), void **arr, int start, int end) //big o o(nlogn)
{
	int j,i=start;
	void *pivot=arr[end];
	void *temp;
	...
	{
		if( (*compare)(pivot,arr[j]) > 0) //if arr elt is less than value of pivot 
		{
			temp=arr[i];//swap
			arr[i]=arr[j];
			arr[j]=temp;
...
void quicksort(int (*compare)(),void **arr, int start, int end) // big o o(nlogn)
{
	int p_ret;//stores return value of the partition function

	if (start < end) //ensures indexes dont coincide
	{
		p_ret=partition(compare,arr,start,end);

		quicksort(compare,arr,start,p_ret-1);//before
		quicksort(compare,arr,p_ret+1,end);//after
	}
}
